*Read this in [Português](README.md).*
...
> Solved flags persist across restarts (saved to `.progress.json`, gitignored).
> Everything else — records, mutations you make (e.g. promoting yourself),
> sessions — is in-memory and resets when you restart the server.
...
```bash
# Ctrl-C the server, then start again — fresh seed data, scoreboard kept.
# To wipe the scoreboard too, delete .progress.json before restarting.
```
